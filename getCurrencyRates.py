...
def getCurrencyRates(c1, c2, time1, time2):
    try:
        now = time.time()
        c1 = c1.upper().strip()
        c2 = c2.upper().strip()
    ...
    for t in data:
        entry = {}
        entry['time'] = float(data[t][0])
        entry['USD%s' % c1] = data[t][1]
        entry['USD%s' % c2] = data[t][2]
        entry['%s%s' % (c1, c2)] = data[t][2] / data[t][1]
        if entry.has_key('USDUSD'): del entry['USDUSD']
        output.append(entry)
    output.sort(key=lambda i: i['time'])
    return output
...
if __name__ == '__main__':
    ...
    print json.dumps(getCurrencyRates(c1, c2, time1, time2))
